...
def _monte_carlo_cached(
    ...
    n_laeufe: int, sigma_items: tuple[tuple[str, float], ...], diskontsatz_pct: float,
):
...
def get_monte_carlo(
    project_id: str, n_laeufe: int, sigmas: dict[str, float], diskontsatz_pct: float
):
    key = _mtimes(project_id)
    if key is None:
        return None
    return _monte_carlo_cached(
        *key, n_laeufe, tuple(sorted(sigmas.items())), diskontsatz_pct
    )
